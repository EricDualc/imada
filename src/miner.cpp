...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setImadaSchedule(imadaDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = imadaDGP.getBlockSize(nHeight);
    minGasPrice = imadaDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = imadaDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    ImadaTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractImadaTX resultConverter;
    if(!convert.extractionImadaTransactions(resultConverter)){
    ...
    for(ImadaTransaction imadaTransaction : imadaTransactions){
        txGas += imadaTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + imadaTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(imadaTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, imadaTransactions, hardBlockGasLimit);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    // Found a solution (stake)
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("IMADAMiner : generated block is stale");

        for(const CTxIn& vin : pblock->vtx[1].vin) {
            if (wallet.IsSpent(vin.prevout.hash, vin.prevout.n)) {
                return error("IMADAMiner : Gen block stake is invalid - UTXO spent");
            }
        }
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("IMADAMiner : ProcessNewBlock, block not accepted");
    }
