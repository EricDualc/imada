...
int nAnonymizeImadaAmount = 1000;
...
{
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // imada
    assert(mutexDebugLog == NULL);

    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // imada
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // imada
    if (fileout) setbuf(fileout, NULL); // unbuffered
    if (fileoutVM) setbuf(fileoutVM, NULL); // unbuffered // imada
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "imada" is a composite category enabling all IMADA-related debug output
            if (ptrCategory->count(string("imada"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\IMADA
// Windows >= Vista: C:\Users\Username\AppData\Roaming\IMADA
// Mac: ~/Library/Application Support/IMADA
// Unix: ~/.imada
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IMADA";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "imada.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty imada.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override imada.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "imadad.pid"));
