...
bool ImadaDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule ImadaDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t ImadaDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t ImadaDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t ImadaDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t ImadaDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool ImadaDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void ImadaDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void ImadaDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void ImadaDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void ImadaDGP::createParamsInstance(){
...
dev::Address ImadaDGP::getAddressForBlock(unsigned int blockHeight){
...
void ImadaDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ImadaDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ImadaDGP::parseStorageOneUint64(uint64_t& value){
...
void ImadaDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule ImadaDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
