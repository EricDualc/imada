...
#include <util.h>
#include <main.h>
#include "imadastate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

ImadaState::ImadaState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = ImadaState::openDB(_path + "/imadaDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ImadaState::ImadaState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute ImadaState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, ImadaTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createImadaAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            imada::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> ImadaState::vins() const // temp
{
...
void ImadaState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* ImadaState::vin(dev::Address const& _a) const
{
    return const_cast<ImadaState*>(this)->vin(_a);
}

Vin* ImadaState::vin(dev::Address const& _addr)
{
...
// void ImadaState::commit(CommitBehaviour _commitBehaviour)
// {
...
void ImadaState::kill(dev::Address _addr)
{
...
void ImadaState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address ImadaState::createImadaAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void ImadaState::deleteAccounts(std::set<dev::Address>& addrs){
...
void ImadaState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void ImadaState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
