...
public:
    ...
    unsigned int nBits;
    unsigned int nNonce;
    uint256 hashStateRoot; // imada
    uint256 hashUTXORoot; // imada
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot  = uint256(); // imada
        hashUTXORoot   = uint256(); // imada
    }
    ...
    CBlockIndex(const CBlock& block)
    {
        ...
        nBits = block.nBits;
        nNonce = block.nNonce;
        hashStateRoot  = block.hashStateRoot; // imada
        hashUTXORoot   = block.hashUTXORoot; // imada
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits = nBits;
        block.nNonce = nNonce;
        block.hashStateRoot  = hashStateRoot; // imada
        block.hashUTXORoot   = hashUTXORoot; // imada
        return block;
    }
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int _nVersion)
    {
        ...
        if ((this->nVersion & (1 << 30)) != 0) {
            READWRITE(hashStateRoot);       // imada
            READWRITE(hashUTXORoot);        // imada
        }
    }
