    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Imada and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002Imada per transaction on average.
            */
            if (r <= 20) {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready) {
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeImadaAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if (!sessionFoundMasternode) {
        ...
        double fImadaSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f Imada CENT - sessionTotalValue %d\n", fImadaSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom) {
    ...
    //
    // bit 0 - 100Imada+1 ( bit on if present )
    // bit 1 - 10Imada+1
    // bit 2 - 1Imada+1
    // bit 3 - .1Imada+1
    // bit 3 - non-denom
        ...
        if (c % 60 * (nLiquidityProvider + 1) == 0) {
            if (nLiquidityProvider != 0) {
                ...
                if (nRand == 50 + nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8) {
                    ...
                    nAnonymizeImadaAmount = (rand() % nLeftToAnon) + 3;
                } else {
