...
void HexAddressConverter::addressChanged(const QString& address) {
    ...
    if(!address.isEmpty()) {
        ...
        hasImadaAddress = IsValidDestination(addr);
        if (hasImadaAddress) {
            ui->label->setText("Address (Imada)");
        ...
        isAddressValid = hasHexAddress || hasImadaAddress;
    }
