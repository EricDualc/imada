...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::IMADA);
    ...
    if (!settings.contains("nAnonymizeImadaAmount"))
        settings.setValue("nAnonymizeImadaAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeImadaAmount = settings.value("nAnonymizeImadaAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeImadaAmount"))
        SoftSetArg("-anonymizeimadaamount", settings.value("nAnonymizeImadaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in imada.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeImadaAmount:
            return QVariant(nAnonymizeImadaAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeImadaAmount:
            nAnonymizeImadaAmount = value.toInt();
            settings.setValue("nAnonymizeImadaAmount", nAnonymizeImadaAmount);
            emit anonymizeImadaAmountChanged(nAnonymizeImadaAmount);
            break;
        case CoinControlFeatures:
