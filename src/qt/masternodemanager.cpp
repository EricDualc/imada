...
static void NotifyImadaNodeUpdated(MasternodeManager *page, CImadaNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateImadaNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyImadaNodeChanged.connect(boost::bind(&NotifyImadaNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyImadaNodeChanged.disconnect(boost::bind(&NotifyImadaNodeUpdated, this, _1));
}
...
void MasternodeManager::updateImadaNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        for (PAIRTYPE(std::string, CImadaNodeConfig) adrenaline : pwalletMain->mapMyImadaNodes)
        {
            updateImadaNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditImadaNode* aenode = new AddEditImadaNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CImadaNodeConfig c = pwalletMain->mapMyImadaNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    ImadaNodeConfigDialog* d = new ImadaNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyImadaNodes.erase(sAddress);
        walletdb.EraseImadaNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    for (PAIRTYPE(std::string, CImadaNodeConfig) adrenaline : pwalletMain->mapMyImadaNodes)
    {
        CImadaNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
