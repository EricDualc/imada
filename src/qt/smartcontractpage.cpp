...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::IMADA)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    if (fLiteMode) {
        ...
        if (fMasterNode) {
            ...
            if (!fEnableImadasend) {
                ui->toggleObfuscation->setText(tr("Start Imadasend"));
            } else {
                ui->toggleObfuscation->setText(tr("Stop Imadasend"));
            }
...
void SmartContractPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("IMADA")
    updateDisplayUnit();
}
...
void SmartContractPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeImadaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeImadaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeImadaAmount = strAnonymizeImadaAmount.remove(strAnonymizeImadaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeImadaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeImadaAmount * COIN) nMaxToAnonymize = nAnonymizeImadaAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeImadaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeImadaAmount));
        strAnonymizeImadaAmount = strAnonymizeImadaAmount.remove(strAnonymizeImadaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeImadaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void SmartContractPage::toggleObfuscation()
{
    ...
    fEnableImadasend = !fEnableImadasend;
    obfuscationPool.cachedNumBlocks = std::numeric_limits<int>::max();
