...
bool ToHash160(const std::string& strImadaAddress, std::string& strHash160)
{   
    CTxDestination imadaAddress = DecodeDestination(strImadaAddress);
    if(IsValidDestination(imadaAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&imadaAddress);
...
bool ToImadaAddress(const std::string& strHash160, std::string& strImadaAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strImadaAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToImadaAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToImadaAddress(tokenEvent.receiver, tokenEvent.receiver);
